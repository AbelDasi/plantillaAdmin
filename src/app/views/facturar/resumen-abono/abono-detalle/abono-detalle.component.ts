...
let rowIndex: number;
@Component({
  selector: 'app-abono-detalle',
  templateUrl: './abono-detalle.component.html',
  styleUrls: ['./abono-detalle.component.scss']
})
export class AbonoDetalleComponent implements OnInit {
...
modificarPerfil() {
                  ...
                  if (ok === data.ok) {
                    if (data.ok) {
                      ...
                      /* esto.facturacion.getDetallesSolicitud(esto.facturacion.oidsArrayx).then(data5 => {
                       ...
                       esto.facturacion.getDetalleAbono(esto.oidAbono).then((data5: any) => {
  ...
  eliminarFilas() {
    confirm('Si estos servicios pertenecian a una factura previa solo seran borrados del abono', '¿Continuar?').then(res => {
      if (res) {
        ...
        if (oids.length > 0) {
          let objetoEliminarAbono: any = {
            oids : oids,
            abono: this.oidAbono
          };
          
          this.facturacion.deleteServiciosAbono(objetoEliminarAbono).then((data: any) => {
            if (data.ok) {
