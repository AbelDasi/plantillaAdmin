...
export class PrefacturaDetalleComponent implements OnInit {
  ...
  perfilHtml = `{"columns":[{"visibleIndex":0,"dataField":"tabla",
...
// string y nuevafila
  ...
  quitarPerfilPredeterminado() {
    this.facturacion.quitarPerfil(this.seleccionado,this.pantalla).then((data: any) => {
      if (data.ok) {
        ...
        sessionStorage.setItem('serviciosFacGrid',JSON.stringify(this.perfilHtml));
    ...
    if (this.detectarEventoChange === 'change') {
      confirm('No ha guardado los cambios que ha realizado, se perderan los datos', '¿Está seguro/a?').then(res => {
        if (res) {
          ...
          this.facturacion.prefacturaBol = true;
  ...
  guardarDatosGrid() {
        ...
        this.grid1.instance.saveEditData().then(() => {
          this.facturacion.loading = true;
          this.mensajeCarga = 'Cargando';
          this.facturacion.modificarServicios(this.filasEditadas).then(data => {
            const ok = data.ok;
            if (ok === data.ok) {
              if (data.ok) {
                this.facturacion.loading = false;
                this.toastr.success('Se han modificado ' + data.numero + ' registros', 'Correcto');
                this.facturacion.getDetallePrefactura(this.oidPrefactura).then((data5: any) => {
                this.facturacion.listaDetalles = data5.servicios;
                this.cambiarCabecera.emit(data5.cabecera);
                this.dataSourcex = new ArrayStore({
                data: this.facturacion.listaDetalles
                  });
                }); 
  ...
  enviarAFacturar() {
    this.facturacion.pantallaModulo = 'servicios';
